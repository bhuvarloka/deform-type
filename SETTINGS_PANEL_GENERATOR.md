...
**`src/assets/style/style.css`** — top of file:
...
**`src/assets/style/settings-panel.css`** — component CSS that Tailwind utilities can't express. Start from this baseline and add only what the project needs:
...
The user provides `src/assets/images/gear.svg` and `src/assets/images/close.svg`. They should be monochrome black so `filter: invert(1)` flips them cleanly between dark and light backgrounds. Do not invent fallback SVGs — if the files are missing, ask the user.
...
// --- Shared state object (defined at module scope, read by the sketch) ---
// Replace keys and defaults with whatever Step 0 produced.
const state = {
  myText: "Hello",
  myNumber: 1,
  myToggle: false,
...
textInput?.addEventListener("keydown", (e) => {
  if (e.key === "Enter") state.myText = textInput.value;
});

// toggle
const toggleEl = document.getElementById("my-toggle");
toggleEl?.addEventListener("change", () => {
  state.myToggle = toggleEl.checked;
});

// number
const numberEl = document.getElementById("my-number");
numberEl?.addEventListener("input", () => {
  state.myNumber = parseFloat(numberEl.value) || 0;
});

// reset — restore defaults and sync all inputs
document.getElementById("reset-btn")?.addEventListener("click", () => {
  Object.assign(state, DEFAULTS);
  if (textInput) textInput.value = state.myText;
  if (toggleEl) toggleEl.checked = state.myToggle;
  if (numberEl) numberEl.value = state.myNumber;
});
